...
int solution(vector<string> storage , vector<string> requests){
    ...
    vector<vector<bool> > container(c , vector<bool>(r , true)); // 해당 위치 컨테이너 존재 여부
    for(string req : requests){
        vector<vector<bool>> copyContainer = container; //현재 컨테이너 저장
        for(int y =0; y <c; y++){
            for(int x =0; x<r; x++){
